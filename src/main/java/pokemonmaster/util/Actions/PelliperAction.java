...
public class PelliperAction extends AbstractGameAction {
    private static int BAIT=0;
    private static boolean TODO = false;

    public PelliperAction(Integer number){
    this.amount = number;
    TODO = false;
        BAIT=0;
    }
        ...
        if (TODO) {
            addToTop(new SelectCardsInHandAction(1, "exhaust", false, true, card -> card.hasTag(CustomTags.BAIT), abstractCards -> {
                for (AbstractCard i : abstractCards) {
                    ...
                    addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new EnergizedBluePower(AbstractDungeon.player, NEWMAGIC)));
