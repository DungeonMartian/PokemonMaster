...
import pokemonmaster.cards.Base.GlaceonGX;
...
public class WaterStone extends CustomPotion {
    public static final String POTION_ID = makeID("WaterStone");
    ...
    public void use(AbstractCreature target) {
        addToBot(new SelectCardsInHandAction(this.potency, "evolve",true,true, card -> card.hasTag(CustomTags.POKEMON) && card.hasTag(CustomTags.WATER) || card.hasTag(CustomTags.EEVEE), cards -> {
            for (AbstractCard c2 : cards) {
                ...
                for (AbstractCard c : (AbstractDungeon.player).masterDeck.group) {
                    if (c.uuid == c2.uuid) {
                        ...
                        if (c2.hasTag(CustomTags.EEVEE)){
                            AbstractDungeon.actionManager.addToBottom(new AddCardToDeckAction(new GlaceonGX()));
                            //AbstractDungeon.player.masterDeck.removeCard(c2);
                            TOREMOVE.add(c);
                            //TOREMOVE = c;
                            //AbstractDungeon.player.masterDeck.removeCard(c);
                ...
                if (c2.hasTag(CustomTags.EEVEE)){
                    AbstractDungeon.actionManager.addToBottom(new MakeTempCardInHandAction(new GlaceonGX(), 1));
