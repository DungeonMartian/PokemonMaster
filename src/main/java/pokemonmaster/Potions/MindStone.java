...
public class MindStone extends CustomPotion {
    public static final String POTION_ID = makeID("MindStone");
    ...
    public void use(AbstractCreature target) {
        addToBot(new SelectCardsInHandAction(this.potency, "evolve",true,true, card -> card.hasTag(CustomTags.POKEMON) && card.hasTag(CustomTags.PSYCHIC) || card.hasTag(CustomTags.EEVEE), cards -> {
            for (AbstractCard c2 : cards) {
                ...
                for (AbstractCard c : (AbstractDungeon.player).masterDeck.group) {
                    if (c.uuid == c2.uuid) {
                        ...
                        if (c2.hasTag(CustomTags.EEVEE)){
                            AbstractDungeon.actionManager.addToBottom(new AddCardToDeckAction(new EspeonGX()));
                            //TOREMOVE = c;
                            TOREMOVE.add(c);
                            //AbstractDungeon.player.masterDeck.removeCard(c2);
                            //AbstractDungeon.player.masterDeck.removeCard(c);
                ...
                if (c2.hasTag(CustomTags.EEVEE)){
                    AbstractDungeon.actionManager.addToBottom(new MakeTempCardInHandAction(new EspeonGX(), 1));
